...
const QuotationForm = ({ onClose }) => {
  ...
  const validateField = (name, value) => {
    ...
    switch (name) {
      ...
      case "customPaymentTerms":
        if (formData.paymentTerms === "Custom" && !value.trim()) error = "Custom payment terms are required"
        break
      default:
        break
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate rfqId is present
    if (!formData.rfqId) {
      newErrors.rfqId = "RFQ ID is required"
    }
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      const submissionData = {
        ...
        paymentTerms: formData.paymentTerms === "Custom" ? formData.customPaymentTerms : formData.paymentTerms,
      ...
      try {
        ...
        const response = await axios.post("http://localhost:3000/api/v1/quotations/create", submissionData, {
          headers: {
        ...
        alert(`Failed to submit quotation: ${error.response?.data?.msg || error.message}`)
      }
    } else {
  ...
  const inputClass =
    "p-1.5 border border-green-300 rounded w-full text-sm text-black bg-white focus:outline-none focus:border-green-600 focus:shadow-[0_0_5px_rgba(22,163,74,0.3)]"
  ...
  return (
    <div className="flex-1 overflow-auto p-6 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white">
      ...
      <form
        ...
        className="w-full m-0 p-5 font-sans bg-white dark:bg-gray-800 box-border flex flex-col md:p-3 sm:p-2 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700"
      >
          ...
          <div className="w-[45%] text-right md:w-full md:text-left">
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-end md:justify-start">
                <label
                  className={`${labelClass} w-[150px] text-right md:text-xs sm:text-[11px] sm:w-[120px] sm:text-left dark:text-gray-300`}
                >
                  Quotation Number:
                </label>
                <div className="flex flex-col">
                  <input
                    ...
                    className={`${inputClass} w-[150px] md:w-[150px] sm:w-full sm:text-[11px] ${
              ...
              <div className="flex items-center justify-end md:justify-start">
                ...
                <div className="flex flex-col">
                  <input
                    ...
                    className={`${inputClass} w-[150px] md:w-[150px] sm:w-full sm:text-[11px] ${
                      errors.rfqId ? "border-red-500" : ""
                    }`}
                  />
                  {errors.rfqId && <span className={errorClass}>{errors.rfqId}</span>}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-green-50 dark:bg-green-900/20 p-4 mb-5 border border-green-300 dark:border-green-800 box-border rounded-lg md:p-2.5 sm:p-2">
          <p>
            <strong className="text-base text-black dark:text-white block mb-2.5 md:text-sm sm:text-[11px] font-bold">
              Customer Section
            </strong>
          </p>
          <div className="flex flex-wrap gap-2.5 items-center">
            <div className="flex items-center mb-1.5 flex-1 min-w-0">
              ...
              <input
                ...
                className={`${inputClass} w-[200px] flex-grow md:text-xs sm:text-[11px] sm:p-1 sm:w-full ${
                  errors.billToCompany ? "border-red-500" : ""
                }`}
              />
            ...
            <div className="flex items-center mb-1.5 flex-1 min-w-0">
              ...
              <input
                ...
                className={`${inputClass} w-[200px] flex-grow md:text-xs sm:text-[11px] sm:p-1 sm:w-full ${
                  errors.billToAddress ? "border-red-500" : ""
                }`}
              />
            ...
            <div className="flex items-center mb-1.5 flex-1 min-w-0">
              ...
              <input
                ...
                className={`${inputClass} w-[200px] flex-grow md:text-xs sm:text-[11px] sm:p-1 sm:w-full ${
                  errors.billToCityState ? "border-red-500" : ""
                }`}
              />
            ...
            <div className="flex items-center mb-1.5 flex-1 min-w-0">
              ...
              <input
                ...
                className={`${inputClass} w-[200px] flex-grow md:text-xs sm:text-[11px] sm:p-1 sm:w-full ${
                  errors.billToPostalCode ? "border-red-500" : ""
                }`}
              />
        ...
        <div className="mb-5">
          <table className="w-full border-collapse mb-2.5">
            <thead>
              <tr>
                <th className="border border-green-300 dark:border-green-800 p-2.5 text-left text-sm font-bold bg-green-50 dark:bg-green-900/20 md:text-xs sm:text-[11px] md:p-2 sm:p-1.5">
                  Description
                </th>
                <th className="border border-green-300 dark:border-green-800 p-2.5 text-left text-sm font-bold bg-green-50 dark:bg-green-900/20 md:text-xs sm:text-[11px] md:p-2 sm:p-1.5">
                  Hours
                ...
                <tr key={index}>
                  <td className="border border-green-300 dark:border-green-800 p-2.5 text-sm md:text-xs sm:text-[11px] md:p-2 sm:p-1.5">
                    <input
                      ...
                      className={`w-full p-1 border border-green-300 dark:border-green-800 rounded text-black md:p-1 sm:p-0.5 ${
                  ...
                  <td className="border border-green-300 dark:border-green-800 p-2.5 text-sm md:text-xs sm:text-[11px] md:p-2 sm:p-1.5">
                    <input
                      ...
                      placeholder="0"
                      className={`w-full p-1 border border-green-300 dark:border-green-800 rounded text-black md:p-1 sm:p-0.5 ${
                        errors[`serviceRate${index}`] ? "border-red-500" : ""
                      }`}
                    />
        ...
        <div className="bg-green-50 dark:bg-green-900/20 p-4 mb-5 border border-green-300 dark:border-green-800 flex flex-col rounded-lg md:p-2.5 sm:p-2">
          ...
          <div className="mb-2.5">
            ...
            <input
              ...
              min="0"
              placeholder="0"
              className={`${inputClass} w-[100px] md:w-[80px] md:text-xs sm:w-[70px] sm:text-[11px] sm:p-1 ${
          ...
          <p className="m-1.5 text-sm md:text-xs sm:text-[11px]">
            <strong className="text-lg md:text-base sm:text-sm">Total: {calculateTotal().toFixed(2)}</strong>
          </p>
        </div>
        <div className="bg-green-50 dark:bg-green-900/20 p-4 mb-5 border border-green-300 dark:border-green-800 w-full box-border rounded-lg md:p-2.5 sm:p-2">
          <p>
            <strong className="text-base text-black dark:text-white block mb-4 font-bold md:text-sm sm:text-[11px]">
              Payment & Terms:
            </strong>
          </p>
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-4 md:flex-col md:items-start md:gap-2 sm:gap-2">
              <label
                className={`${labelClass} w-[150px] text-right md:text-xs sm:text-[11px] sm:w-full sm:text-left dark:text-gray-300`}
              >
                Payment Terms:
              </label>
              <div className="flex-1">
                <select
                  ...
                  className={`${inputClass} w-full md:text-xs sm:text-[11px] sm:p-1 ${
              ...
              <div className="flex items-start gap-4 md:flex-col md:items-start md:gap-2 sm:gap-2">
                ...
                <div className="flex-1">
                  <textarea
                    ...
                    rows="3"
                    className={`${inputClass} w-full md:p-1 sm:p-0.5 ${
                      errors.customPaymentTerms ? "border-red-500" : ""
                    }`}
                  />
                  {errors.customPaymentTerms && <span className={errorClass}>{errors.customPaymentTerms}</span>}
                </div>
              </div>
            )}
          </div>
        ...
        <div className="bg-green-50 dark:bg-green-900/20 p-4 mb-5 border border-green-300 dark:border-green-800 rounded-lg md:p-2.5 sm:p-2">
          <label htmlFor="notes" className={`${labelClass} block mb-1.5 md:text-xs sm:text-[11px] dark:text-gray-300`}>
            <strong>Notes:</strong>
          </label>
          <textarea
            id="notes"
        ...
        <div className="bg-green-50 dark:bg-green-900/20 p-4 mb-5 border border-green-300 dark:border-green-800 w-full box-border rounded-lg md:p-2.5 sm:p-2">
          <p>
            <strong className="text-base text-black dark:text-white block mb-2.5 font-bold md:text-sm sm:text-[11px]">
          ...
          <ol className="pl-5 m-0 text-sm text-gray-700 dark:text-gray-300 md:text-xs md:pl-4 sm:text-[11px] sm:pl-2.5">
            ...
            <li className="mb-1.5 md:mb-1 sm:mb-0.5">
              3. All services are subject to the terms agreed upon in the contract.
            </li>
            <li className="mb-1.5 md:mb-1 sm:mb-0.5">
              4. Tech Elecon Pvt Ltd reserves the right to modify these terms with prior notice.
            </li>
          </ol>
        </div>
